...
pageMod.PageMod({
	...
	onAttach: function(worker)
	{
		worker.port.on('setStorage', function(payload)
        {
            ...
            {
                worker.port.emit(payload.callbackName, {data: storageObject.storage});
            }
        });

        worker.port.on('getStorage', function(callbackName)
        {
            worker.port.emit(callbackName, storageObject.storage);
        });
    }
});